(define (domain blocks-domain)
  ...
  (:action put-on-block
    :parameters (?b1 ?b2 - block)
    :precondition (and (holding ?b1) (clear ?b2) (clear ?b1))
    :effect (oneof (and (on ?b1 ?b2) (emptyhand) (not (holding ?b1)) (not (clear ?b2)))
                   (and (on-table ?b1) (emptyhand) (not (holding ?b1))))
  )
  (:action put-down
    :parameters (?b - block)
  ...
  (:action pick-tower
    ...
    :effect
      (oneof (and) (and (not (clear ?b1)) (holding ?b2) (clear ?b3) (not (emptyhand)) (not (on ?b2 ?b3))))
  )
  (:action put-tower-on-block
    :parameters (?b1 ?b2 ?b3 - block)
    :precondition (and (holding ?b2) (on ?b1 ?b2) (clear ?b3))
    :effect (oneof (and (on ?b2 ?b3) (clear ?b1) (emptyhand) (not (holding ?b2)) (not (clear ?b3)))
                   (and (on-table ?b2) (clear ?b1) (emptyhand) (not (holding ?b2))))
  )
  (:action put-tower-down
    :parameters (?b1 ?b2 - block)
    :precondition (and (holding ?b2) (on ?b1 ?b2))
    :effect (and (on-table ?b2) (clear ?b1) (emptyhand) (not (holding ?b2)))
  )
)
